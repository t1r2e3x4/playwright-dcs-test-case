...
class ProductPageMixin:
    ...
    def product_dialog(self, product_name: str) -> Locator:
        return self.page.locator(
            f"div[data-testid='dialog-paper']:has(h5:text('{product_name}'))"
        )

    def option_div(self, option_category: str) -> Locator:
        option_heading = self.product_dialog_div.locator(
            f"h6:has-text('{option_category}')"
        )
    ...
    def option_btn(self, option_category: str, option_name: str) -> Locator:
        return (
            self.options_in_option_div(option_category)
            .locator(f"p:text('{option_name}')")
            .locator("..")
        )
